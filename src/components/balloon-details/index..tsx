...
export const BalloonDetails: React.FC<Balloon> = (balloon) => {
  ...
  const addToCart = () => {
    const products: BallonDetails[] = JSON.parse(localStorage.getItem('cart') || "[]")
    ...
    if (existingpProduct) {
      existingpProduct.quantity = existingpProduct.quantity ? existingpProduct.quantity + qty : qty
      existingpProduct.cost = existingpProduct.cost ? existingpProduct.cost + cost : cost
    } else {
